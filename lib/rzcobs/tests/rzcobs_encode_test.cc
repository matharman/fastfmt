#include <gtest/gtest.h>
#include <gmock/gmock.h>
#include <array>

#include "rzcobs.h"

TEST(rzCOBS, init) {
    auto bytesink = [] (void *ctx, const void *buf, size_t len){
        return 0;
    };

    RZCobWriter writer;
    rzcob_writer_init(&writer, bytesink, nullptr);

    ASSERT_EQ(writer.run, 0);
    ASSERT_EQ(writer.zeros, 0);
}

TEST(rzCOBS, framing) {
        struct test_case {
            std::vector<uint8_t> src;
            std::vector<uint8_t> result;
        };

        struct test_case cases[] = {
            {{}, {0x00}},
            {{0x00}, {0x7f, 0x00}},
            {{0x00, 0x00}, {0x7f, 0x00}},
            {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, {0x7f, 0x00}},
            {
                {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
                {0x7f, 0x7f, 0x00},
            },
            {{0x01}, {0x01, 0x7e, 0x00}},
            {{0x01, 0x00}, {0x01, 0x7e, 0x00}},
            {{0x00, 0x01}, {0x01, 0x7d, 0x00}},
            {{0x01, 0x02}, {0x01, 0x02, 0x7c, 0x00}},
            {
                {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x00},
                {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x40, 0x00},
            },
            {
                {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77},
                {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x80, 0x00},
            },
            {
                {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x00},
                {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x80, 0x00},
            },
            {
                {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88},
                {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x81, 0x00},
            },
            {
                {
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0xff,
                },
                {0x7f, 0xff, 0x3f, 0x00},
            },
            {
                {
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x44, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0xff,
                },
                {0x44, 0x5f, 0xff, 0x3f, 0x00},
            },
            {
                {
                    ,
                },
                {
                    ,
                },
            },
            {
                {
                    ,
                },
                {
                    ,
                },
            },
            {
                {
                    ,
                },
                {
                    ,
                },
            },
            {
                {
                    ,
                },
                {
                    ,
                },
            },
        };

        auto bytesink = [] (void *ctx, const void *buf, size_t len){
            std::vector<uint8_t> *ptr = static_cast<std::vector<uint8_t> *>(ctx);
            const uint8_t *data = reinterpret_cast<const uint8_t *>(buf);
            for (auto i = 0; i < len; i++) {
                ptr->push_back(data[i]);
            }
            return 0;
        };

        for (auto i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
            std::cout << "case " << i << std::endl;
            std::vector<uint8_t> result;

            RZCobWriter writer;
            rzcob_writer_init(&writer, bytesink, &result);
            writer.started = 1;

            rzcob_start_frame(&writer);
            rzcob_write(&writer, cases[i].src.data(), cases[i].src.size());
            rzcob_end_frame(&writer);

            ASSERT_THAT(result, ::testing::ElementsAreArray(cases[i].result));
        }
    }
