package rzcobs

import (
	"bytes"
	"testing"
)

func TestRZCOBSDecode(t *testing.T) {
	testCases := []struct {
		plain   []byte
		encoded []byte
	}{
		{[]byte{}, []byte{0x00}},
		{[]byte{0x00}, []byte{0x7f, 0x00}},
		{[]byte{0x00, 0x00}, []byte{0x7f, 0x00}},
		{[]byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, []byte{0x7f, 0x00}},
		{
			[]byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
			[]byte{0x7f, 0x7f, 0x00},
		},
		{[]byte{0x01}, []byte{0x01, 0x7e, 0x00}},
		{[]byte{0x01, 0x00}, []byte{0x01, 0x7e, 0x00}},
		{[]byte{0x00, 0x01}, []byte{0x01, 0x7d, 0x00}},
		{[]byte{0x01, 0x02}, []byte{0x01, 0x02, 0x7c, 0x00}},
		{
			[]byte{0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x00},
			[]byte{0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x40, 0x00},
		},
		{
			[]byte{0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77},
			[]byte{0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x80, 0x00},
		},
		{
			[]byte{0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x00},
			[]byte{0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x80, 0x00},
		},
		{
			[]byte{0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88},
			[]byte{0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x81, 0x00},
		},
		{
			[]byte{
				0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				0xff,
			},
			[]byte{0x7f, 0xff, 0x3f, 0x00},
		},
		{
			[]byte{
				0x00, 0x00, 0x00, 0x00, 0x00, 0x44, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				0xff,
			},
			[]byte{0x44, 0x5f, 0xff, 0x3f, 0x00},
		},
		{
			[]byte{
				,
			},
			[]byte{
				,
			},
		},
		{
			[]byte{
				,
			},
			[]byte{
				,
			},
		},
		{
			[]byte{
				,
			},
			[]byte{
				,
			},
		},
		{
			[]byte{
				,
			},
			[]byte{
				,
			},
		},
	}

	for _, tc := range testCases {
		decoded, err := Decode(tc.encoded)
		if err != nil {
			t.Failed()
			return
		}

		if !bytes.Equal(decoded, tc.plain) {
			t.Failed()
			return
		}
	}
}
